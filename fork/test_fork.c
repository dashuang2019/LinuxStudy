...
#define EAaFileRet int
#define EAaFileRet_Error 0
#define EAaFileRet_OK 1
#define EAaFileRet_NOTOK 2
#define u32 unsigned int
// input = "tar -zxf pycrate-0.4.tar.gz -C /var/fpwork/dmeng/source_codes/fork/test"
EAaFileRet executeCmd(const char *cmd, char* const input[]) {
  pid_t pid;
  char **env;
  extern char **environ;
  char filename[256] = {};
  ...
  else if (pid > 0)
  {
     ...
     retval = EAaFileRet_OK;
     _Exit(0);
  }
  else
  {
     printf("The parent process. return pid=%d.\n", pid);
     printf("The parent process. pid=%d.\n", getpid());
    strcpy(filename, "/usr/bin/");
    strcat(filename, cmd);
    env = environ;
    if (execve(filename, input, env) != -1) {
      retval = EAaFileRet_NOTOK;
    }
    retval = EAaFileRet_NOTOK;
  }
  return retval;
...
int testfork()
{
    printf("testfork addr is : %p .", testfork);
    int i=0;  
...
void testchdir(void)
{
    printf("current in testchdir....\n");
    //system("ps -aux > test1.txg;");
    //system("ps -aux > test2.txg; sleep 5; ps -aux > test3.txg;");
    //system("ps -aux > test4.txg;");
    if(chdir("/tmp") < 0)
    {
        perror("chdir");
        exit(1);
    ...
    system("pwd");
    printf("exit from testchdir....\n");
}

EAaFileRet AaFileTarGzipItemArray(char* archivePath, char** itemPathArray, u32 itemPathArraySize, char* relativeDirPath)
{
    if (itemPathArraySize > 0)
    {
        #define MAX_FILE_LIST_LENGTH 2048
        char fileList[MAX_FILE_LIST_LENGTH] = {};
        u32 pathLength = 0;
        int i=0;
        for (; i<itemPathArraySize; i++)
        {
            memcpy(fileList + pathLength, *(itemPathArray + i), strlen(*(itemPathArray + i)));
            pathLength += strlen(*(itemPathArray + i));
            fileList[pathLength] = ' ';
            pathLength += 1;
            if (pathLength >= MAX_FILE_LIST_LENGTH)
                break;
        }
        printf("fileList=%s.\n", fileList);
        char* cmd = (char*)"tar";
        char* opt = (char*)"-czPf";
        char* optDir = (char*)"-C";
        char* input[] = {cmd, opt, archivePath, fileList, NULL, NULL, NULL};
        if (relativeDirPath != NULL)
        {
            input[4] = optDir;
            input[5] = relativeDirPath;
            input[6] = NULL;
        }
        return executeCmd(cmd, input);
    }
...
int main(int arg, char **args)
{
    ...
    //#define MAX_FILE_LIST_LENGTH 10240
    //char fileList[MAX_FILE_LIST_LENGTH] = {0};
    unsigned int itemPathArraySize = 3;
    char targetFileName[] = {"test_tar.tar.gz"};
    //unsigned int pathLength = 0;
    char* itemPathArray[] = {"/var/fpwork/dmeng/source_codes/fork/test/111.txt",
                            "/var/fpwork/dmeng/source_codes/fork/test/222.txt",
                            "/var/fpwork/dmeng/source_codes/fork/test/333.txt"};
    /*int i=0;
    for (; i<itemPathArraySize; i++)
    {
        memcpy(fileList + pathLength, itemPathArray[i], strlen(itemPathArray[i]));
    ...
    AaFileTarGzipItemArray(targetFileName, itemPathArray, itemPathArraySize, NULL);
    return 1;/*
    char* cmd = static_cast<char*>("tar");
    char* opt = static_cast<char*>("-zxf");
    char* optDir = static_cast<char*>("-C");
    char* filen = static_cast<char*>("pycrate-0.4.tar.gz");
    char* targetDir = static_cast<char*>("/var/fpwork/dmeng/source_codes/fork/test");
    char* const input[] = {cmd, opt, filen, optDir, targetDir, NULL};
    int ret = executeCmd(cmd, input);
    if (ret == 0)
    {
        printf("Unzip failed.\n");
        return 0;
